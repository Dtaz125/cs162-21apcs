...
void inputToVector(Class*& classname, const string _inputfile) {
	...
	for (int i = 0; i < content.size(); i++) {							//loops for every line
		...
		new_user.birth.day = stoi(content[i][5]);			//this is where convert to string function is needed. because day month year members are all int datatype
...
int main() {
	...
	Class* class20apcs1Pointer = &class20apcs1;				//i have to create a pointer, in order to use the function void setClass(Class* _class) in Student Class
